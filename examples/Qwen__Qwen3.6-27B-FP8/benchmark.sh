...
# Benchmark script for Qwen/Qwen3.6-27B-FP8
#
# Prerequisites:
#   pip install -e ".[dev]"
#
...
#   bash examples/benchmark_qwen27b_fp8.sh
#
#   # Reuse an existing server (avoids cold-start per combo):
#   bash examples/benchmark_qwen27b_fp8.sh --no-server --port 8081
#
#   # Only plot existing results:
#   bash examples/benchmark_qwen27b_fp8.sh --plot-only
#
#   # Custom rates, concurrencies, port:
#   bash examples/benchmark_qwen27b_fp8.sh --rates "8.0 16.0" --concurrencies "1 4 8 16" --port 8081
